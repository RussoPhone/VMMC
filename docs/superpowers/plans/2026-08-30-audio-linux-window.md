...
- Tracked bytecode must be removed from the index because verification proved it can block imports; local generated copies remain disposable and ignored.
...
Add `.venv/`, `venv/`, `__pycache__/`, `*.py[cod]`, `.pytest_cache/`, build artifacts, and Aider local state to `.gitignore`. Remove tracked bytecode from the index with `git rm --cached` after preserving local generated copies.
...
Expected: generated bytecode is ignored and absent from the index; implementation and documentation commits are present.
